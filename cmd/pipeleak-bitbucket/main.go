...
func newRootCmd() *cobra.Command {
	...
	bbCmd.Long = `Pipeleak-BitBucket is a tool designed to scan BitBucket Pipelines job output logs and artifacts for potential secrets.

This is a standalone binary for BitBucket-specific functionality.`
	bbCmd.Version = Version
	bbCmd.GroupID = ""

	bbCmd.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		initLogger(cmd)
		setGlobalLogLevel(cmd)
		go logging.ShortcutListeners(nil)
	}

	bbCmd.PersistentFlags().BoolVarP(&JsonLogoutput, "json", "", false, "Use JSON as log output format")
	bbCmd.PersistentFlags().StringVarP(&LogFile, "logfile", "l", "", "Log output to a file")
	bbCmd.PersistentFlags().BoolVarP(&LogDebug, "verbose", "v", false, "Enable debug logging (shortcut for --log-level=debug)")
	bbCmd.PersistentFlags().StringVar(&LogLevel, "log-level", "", "Set log level globally (debug, info, warn, error). Example: --log-level=warn")
	bbCmd.PersistentFlags().BoolVar(&LogColor, "color", true, "Enable colored log output (auto-disabled when using --logfile)")
