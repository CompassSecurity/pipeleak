...
func newRootCmd() *cobra.Command {
	...
	giteaCmd.Long = `Pipeleak-Gitea is a tool designed to scan Gitea Actions job output logs and artifacts for potential secrets.

This is a standalone binary for Gitea-specific functionality.`
	giteaCmd.Version = Version
	giteaCmd.GroupID = ""

	giteaCmd.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		initLogger(cmd)
		setGlobalLogLevel(cmd)
		go logging.ShortcutListeners(nil)
	}

	giteaCmd.PersistentFlags().BoolVarP(&JsonLogoutput, "json", "", false, "Use JSON as log output format")
	giteaCmd.PersistentFlags().StringVarP(&LogFile, "logfile", "l", "", "Log output to a file")
	giteaCmd.PersistentFlags().BoolVarP(&LogDebug, "verbose", "v", false, "Enable debug logging (shortcut for --log-level=debug)")
	giteaCmd.PersistentFlags().StringVar(&LogLevel, "log-level", "", "Set log level globally (debug, info, warn, error). Example: --log-level=warn")
	giteaCmd.PersistentFlags().BoolVar(&LogColor, "color", true, "Enable colored log output (auto-disabled when using --logfile)")
