...
func newRootCmd() *cobra.Command {
	...
	ghCmd.Long = `Pipeleak-GitHub is a tool designed to scan GitHub Actions job output logs and artifacts for potential secrets.

This is a standalone binary for GitHub-specific functionality.`
	ghCmd.Version = Version
	ghCmd.GroupID = ""

	ghCmd.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		initLogger(cmd)
		setGlobalLogLevel(cmd)
		go logging.ShortcutListeners(nil)
	}

	ghCmd.PersistentFlags().BoolVarP(&JsonLogoutput, "json", "", false, "Use JSON as log output format")
	ghCmd.PersistentFlags().StringVarP(&LogFile, "logfile", "l", "", "Log output to a file")
	ghCmd.PersistentFlags().BoolVarP(&LogDebug, "verbose", "v", false, "Enable debug logging (shortcut for --log-level=debug)")
	ghCmd.PersistentFlags().StringVar(&LogLevel, "log-level", "", "Set log level globally (debug, info, warn, error). Example: --log-level=warn")
	ghCmd.PersistentFlags().BoolVar(&LogColor, "color", true, "Enable colored log output (auto-disabled when using --logfile)")
