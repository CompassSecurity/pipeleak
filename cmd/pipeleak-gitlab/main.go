...
func newRootCmd() *cobra.Command {
	...
	glCmd.Long = `Pipeleak-GitLab is a tool designed to scan GitLab CI/CD job output logs and artifacts for potential secrets.

This is a standalone binary for GitLab-specific functionality.`
	glCmd.Version = Version
	glCmd.GroupID = ""

	glCmd.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		initLogger(cmd)
		setGlobalLogLevel(cmd)
		go logging.ShortcutListeners(nil)
	}

	glCmd.PersistentFlags().BoolVarP(&JsonLogoutput, "json", "", false, "Use JSON as log output format")
	glCmd.PersistentFlags().StringVarP(&LogFile, "logfile", "l", "", "Log output to a file")
	glCmd.PersistentFlags().BoolVarP(&LogDebug, "verbose", "v", false, "Enable debug logging (shortcut for --log-level=debug)")
	glCmd.PersistentFlags().StringVar(&LogLevel, "log-level", "", "Set log level globally (debug, info, warn, error). Example: --log-level=warn")
	glCmd.PersistentFlags().BoolVar(&LogColor, "color", true, "Enable colored log output (auto-disabled when using --logfile)")
