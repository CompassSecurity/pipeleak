...
// BuildPipelineStepArtifactURL constructs a web URL for a pipeline step artifact
func BuildPipelineStepArtifactURL(baseWebURL, workspaceSlug, repoSlug string, buildNumber int, stepUUID string) string {
	if baseWebURL == "" {
		baseWebURL = "https://bitbucket.org"
	}
	u, err := url.Parse(baseWebURL)
	...
	u.Path = path.Join(u.Path, "repositories", workspaceSlug, repoSlug, "pipelines", "results", strconv.Itoa(buildNumber), "steps", stepUUID, "artifacts")
	return u.String()
}
